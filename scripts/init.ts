...
async function loadPackageJson(cwd: string): Promise<{ pkg: PackageJson; pkgPath: string }> {
...
async function resolvePreset(options: {
  ...
  if (presetArg) {
    if (!AVAILABLE_PRESETS.includes(presetArg)) {
      throw new Error(`Unknown preset "${presetArg}". Available: ${AVAILABLE_PRESETS.join(', ')}.`)
    }
...
async function ensureDevDependencies(
  ...
  if (!hasPreset) {
    ...
    const existingMin = existingRange ? minVersion(existingRange) : null
    if (existingMin && lt(existingMin, currentPresetVersion)) {
      toInstall.push({ name: PACKAGE_NAME, version: presetSpec })
    }
  }
...
async function ensureTypecheckScript(pkg: PackageJson, pkgPath: string) {
  ...
  if (scripts.typecheck === 'tsc') return

  let message = 'Add package.json script "typecheck": "tsc"?'
  if (scripts.typecheck && scripts.typecheck !== 'tsc') {
    message = `Replace existing "typecheck" script (${scripts.typecheck}) with "tsc"?`
  }
  ...
  pkg.scripts = { ...scripts, typecheck: 'tsc' }
  await writePackageJson(pkgPath, pkg)
  logStep('Set script "typecheck": "tsc"', 'ok')
}
...
async function getCurrentPackageVersion(): Promise<string | null> {
  ...
  try {
    ...
    if (parsed.version && valid(parsed.version)) {
